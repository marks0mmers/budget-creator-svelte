...
const selectedBudget = derived(
    ...
    ([$budgets, $selectedBudgetId]) => $selectedBudgetId ? $budgets.get($selectedBudgetId) : undefined
)

selectedBudgetId.subscribe(budgetId => localStorage.setItem("selectedBudget", budgetId?.toString() ?? ""));
