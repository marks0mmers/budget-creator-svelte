...
const addIncomeSource = async (budgetId: number, incomeSourceContract: UpsertIncomeSourceContract) => {
...
const updateIncomeSource = async (budgetId: number, incomeSourceId: number, incomeSourceContract: UpsertIncomeSourceContract) => {
    const res = await http(`/api/budgets/${budgetId}/incomeSource/${incomeSourceId}`, "PUT", incomeSourceContract);
    const budget: BudgetContract = await res.json();
    budgetStore.budgets.update(budgets => budgets.set(budget.id, Budget.fromContract(budget)));
}
