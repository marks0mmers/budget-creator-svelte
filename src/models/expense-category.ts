...
export class ExpenseCategory implements ExpenseCategoryRecord {
    ...
    private constructor(id: number, name: string, description: string, subCategories: Map<number, ExpenseSubCategory>) {
        this.id = id;
        this.name = name;
    ...
    public static readonly fromContract = (e: ExpenseCategoryContract) => new ExpenseCategory(
        ...
        e.name,
        e.description,
        e.subCategories.reduce((map, e) => map.set(e.id, ExpenseSubCategory.fromContract(e)), new Map<number, ExpenseSubCategory>()),
    )
}
